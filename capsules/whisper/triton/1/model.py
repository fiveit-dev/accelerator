...
class TritonPythonModel:
    def __init__(
        self, device="cuda", model_name="/mnt/models/whisper_large/1/whisper_model"
    ) -> None:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            transcribed_text = self.transcriber_pipeline(
                {"sampling_rate": sampling_rate, "raw": audio}
            )["text"]
            self.logger.log_info(f"Transcribed Text: {transcribed_text}...")
            inference_response = pb_utils.InferenceResponse(
                output_tensors=[
                    pb_utils.Tensor(
                        "transcribed_text", np.array([transcribed_text.encode()])
                    )
                ]
            )
